...
if(request.getParameter("submit") != null){
	...
	if(message == null){
		...
		else if(!(user.phone).matches("[1-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]")){
			message="Phone Number Should contain 10 digits";		
		}
		else if(!(user.name).matches("[A-Z][a-z]+ [A-Z][a-z]*")){
			message="Please enter a valid full name!";		
		}
		else if(!(user.email).matches("[_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@[A-Za-z0-9-]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})")){
