...
<%
	...
	if(totalSeat != null && date != null && destination != null && actionType.equals("search")){
		...
		if(tickDetails.get("is_avaiable").equals("yes")){
			...
			session.setAttribute("from",stationFrom.name);
			session.setAttribute("to",stationTo.name);
			session.setAttribute("date",tickDetails.get("j_date"));
			session.setAttribute("tname",tickDetails.get("train_name"));
		...
		<div class="ticket_info">
			...
			<table class="table">
				<tr>
					...
					<td>
						<ul>
							<li><strong>Fare</strong> <span><%= Double.toString(totalamount) + " Rs" %></span></li>
							<li><strong>Bank Charge</strong> <span><%= bankCharge + " Rs" %></span></li>
							<li><strong>Total Amount</strong> <span><%= (totalamount+bankCharge)+ " Rs" %></span></li>
						</ul>
					</td>
				</tr>
			</table>
