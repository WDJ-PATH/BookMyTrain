...
<form class="ticket_selecting_form" action="<%= Helper.baseUrl %>FindTicket.jsp">
	...
	<table class="table rs_shadow">
		...
		%>
		<% session.setAttribute("tno", tempTrain.get("code"));
		   session.setAttribute("time", tempTrain.get("time"));
		   session.setAttribute("fare", tempTrain.get("fare"));
		   session.setAttribute("coach", tempTrain.get("coach"));
		   %>
		<tr>
			<td><%= i+1 %></td>
